...
def summarize_articles(articles: list[dict], config: dict) -> list[dict]:
    ...
    prompt = f"""以下はテック・AI・Anthropic・音楽・DTM・ビジネス・経済分野の最新記事です。
...
記事のカテゴリ欄が「ビジネス経済」の場合、内容を読んで以下のルールで振り分けてください：
- "ビジネス"：企業戦略・事業事例・スタートアップ・マーケティング・収益モデル・個人事業・フリーランス・マネタイズ・副業・起業に関する内容
- "経済"：マクロ経済・マーケット・産業動向・景気・株・為替・金融政策・地政学リスク・産業構造の変化に関する内容
- null（除外）：純粋な組織内マネジメント・採用・チームビルディングに終始する内容のみ除外（ビジネス・経済どちらかに少しでも該当すれば除外しない）

nullになった記事はJSONに含めないでください。

## 出力形式（JSONのみ、説明文不要）
[
  {{
    "article_index": 記事番号（整数）,
    "category": "テック・AI／Anthropic／音楽／DTM／ビジネス／経済 のいずれか",
    "title": "記事タイトルの日本語訳",
    "importance": "🔴 または 🟡 または 🟢",
